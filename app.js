...
betsGrid.innerHTML="";
data.forEach(row=>{
let estProb=0.6; // placeholder estimation
let fairOdds=(1/estProb).toFixed(2);
let value=((row.odds-fairOdds)/fairOdds*100).toFixed(1);
const card=document.createElement("div");
card.className="card";
card.innerHTML=`
<h3>${row.match} ${value>3?'<span class="value-badge">+'+value+'%</span>':''}</h3>
<p>${row.market} • ${row.bet_date}</p>
<p>Odds: ${row.odds}</p>
<button onclick='customStake(${JSON.stringify(row)})'>Add to Tracker</button>
`;
betsGrid.appendChild(card);
});
}

async function customStake(row){
let stake=prompt("Enter stake amount:",10);
if(!stake) return;
await client.from("bet_tracker").insert({
match:row.match,market:row.market,odds:row.odds,stake:parseFloat(stake),result:"pending"
});
loadTracker();
}
...
winsElem.innerText=wins;lossesElem.innerText=losses;
avgOddsElem.innerText=data.length?(totalOdds/data.length).toFixed(2):0;

profitCard.classList.remove("glow-green","glow-red");
profit>0?profitCard.classList.add("glow-green"):profit<0?profitCard.classList.add("glow-red"):null;
...
chart=new Chart(chartCtx={ctx:document.getElementById("chart").getContext("2d")},{
type:"line",
data:{labels:history.map((_,i)=>i+1),
datasets:[{data:history,tension:0.4,fill:true,
...
data.forEach(r=>{csv+=`${r.match},${r.market},${r.odds},${r.stake},${r.result}\n`;});
const blob=new Blob([csv],{type:"text/csv"});
const url=URL.createObjectURL(blob);
const a=document.createElement("a");a.href=url;a.download="bet_tracker.csv";a.click();
});
}
